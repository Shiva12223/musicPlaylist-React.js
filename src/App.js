...
class App extends Component {
  ...
  getFilteredSongsList = () => {
    const {searchInput, songsList} = this.state
    const filteredSongs = songsList.filter(eachSong =>
  ...
  deleteSong = songId => {
    this.setState(prevState => ({
      songsList: prevState.songsList.filter(eachSong => eachSong.id !== songId),
    }))
  }

  render() {
    const filteredSongsList = this.getFilteredSongsList()
          ...
          <div className="header">
            ...
            <div className="input-search-container">
              <input
                ...
                placeholder="Search"
              />
              <IoIosSearch color="#cbd5e1" size={25} />
            </div>
          </div>
          {filteredSongsList.length > 1 ? (
            <ul className="song-list-container">
              {filteredSongsList.map(eachSong => (
                <SongListItem
                  key={eachSong.id}
                  songDetails={eachSong}
                  deleteSongItem={this.deleteSong}
                />
              ))}
            </ul>
          ) : (
